      ...
      <el-form-item label="平台属性">
        <el-form :inline="true" ref="form" label-width="80px">
          <el-form-item :label="attr.attrName" v-for="(attr,index) in attrInfoList" :key="attr.id">
            <el-select placeholder="请选择" v-model="attr.attrIdAndValueIdAndValueName">
              <el-option :label="attrValue.valueName" :value="`${attr.id}:${attrValue.id}:${attrValue.valueName}`"
...
export default {
  ...
  methods: {
    ...
    async getData(category1Id, category2Id, spu) {
      ...
      if (result1.code == 200) {
        let list_ = result1.data;
        // 0代表设置默认，1代表默认
        list_.forEach(item=>{
          item.isDefault = 0;
        })
        this.spuImageList = list_;
      }
    ...
    handleSelectionChange(params) {
      // 目前收集的数据缺少isDefault字段
      this.imageList = params;
    },

    // 排他操作
    changeDefault(row) {
      // 将所有图片列表数据的isDefault字段变为0
    ...
    async save() {
      ...
      //   if (item.attrIdAndValueIdAndValueName) {
      //     const [attrId, valueId, valueName] = item.attrIdAndValueIdAndValueName.split(":");
      //     // 携带给服务器的参数应该是一个对象
      //     let obj = {attrId, valueId, valueName};
      //     console.log(obj);
      //     arr.push(obj);
      //   }
      // });

      // 将整理好的参数赋值给skuInfo.skuAttrValueList字段
      ...
      skuInfo.skuAttrValueList = attrInfoList.reduce((prev, item)=>{
        if (item.attrIdAndValueIdAndValueName) {
          const [attrId, valueId, valueName] = item.attrIdAndValueIdAndValueName.split(":");
          prev.push({attrId, valueId, valueName})
        }
        return prev;
      }, []);
      ...
      // 4.发请求将收集到的数据传给服务器
      let result_ = await this.$API.sku.reqAddSku(skuInfo);
      if (result_.code == 200) {
